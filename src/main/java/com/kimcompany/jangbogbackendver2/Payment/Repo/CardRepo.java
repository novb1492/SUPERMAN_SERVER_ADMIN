...
public interface CardRepo extends JpaRepository<CardEntity,Long> {
    ...
    @Modifying
    @Query("update CardEntity c set c.P_CARD_APPLPRICE=:price,c.commonPaymentEntity.prtcCnt=:count where c.id=:id and c.commonPaymentEntity.storeEntity.id=:storeId")
